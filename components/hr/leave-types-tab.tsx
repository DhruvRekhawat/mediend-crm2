...
export function LeaveTypesTab() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: Error) => toast.error(error.message || 'Failed to create leave type'),
  })
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: Error) => toast.error(error.message || 'Failed to update leave type'),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: Error) => toast.error(error.message || 'Failed to delete leave type'),
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) setEditingLeaveType(null) }}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{editingLeaveType ? 'Update leave type details' : 'Add a new leave type'}</DialogDescription>
            </DialogHeader>
            <LeaveTypeForm
              leaveType={editingLeaveType}
              onSubmit={(data) => {
                if (editingLeaveType) updateMutation.mutate({ id: editingLeaveType.id, data })
                  ...
                  <TableRow key={leaveType.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => { setEditingLeaveType(leaveType); setIsDialogOpen(true) }}>
...
function LeaveTypeForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit({ name: formData.name, maxDays: formData.maxDays, isActive: formData.isActive })
    if (!leaveType) setFormData({ name: '', maxDays: 0, isActive: true })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <Label>Name</Label>
        <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="e.g., Casual, Paid, Sick" />
      </div>
      <div>
      ...
      <div className="flex items-center space-x-2">
        ...
        <Label htmlFor="isActive" className="cursor-pointer">Active (visible to employees)</Label>
      </div>
      <div className="flex justify-end gap-2">
        <Button type="submit" disabled={isLoading}>{isLoading ? 'Saving...' : leaveType ? 'Update' : 'Create'}</Button>
      </div>
    </form>
  )
}
