...
export function LeaveBalancesTab() {
  ...
  const openEdit = (emp: EmployeeBalance) => {
    ...
    emp.balances.forEach((b) => {
      edits[b.leaveTypeId] = { allocated: b.allocated, used: b.used, remaining: b.remaining }
    })
              ...
              <TableBody>
                {list.map((emp) => (
                  <TableRow key={emp.id}>
                    ...
                    <TableCell>
                      {emp.inProbation ? <Badge variant="secondary">Probation (0 leave)</Badge> : <Badge variant="outline">Active</Badge>}
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={!!editing} onOpenChange={() => setEditing(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>{editing?.name} ({editing?.employeeCode})</DialogDescription>
          </DialogHeader>
          {editing && (
            <div className="space-y-4">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setBalanceEdits((prev) => ({
                        ...prev,
                        [b.leaveTypeId]: {
                          ...(prev[b.leaveTypeId] ?? { allocated: b.allocated, used: b.used, remaining: b.remaining }),
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                {editing.balances.map((b) => {
                  const e = balanceEdits[b.leaveTypeId] ?? { allocated: b.allocated, used: b.used, remaining: b.remaining }
                  return (
                    <Button
                      key={b.leaveTypeId}
                      onClick={() =>
